...
contract LimitingPaymaster is BasePaymaster {
        ...
        returns (bytes memory context, uint256 validationData)
    {
        ...
        bytes32 hash =
            ECDSA.toEthSignedMessageHash(getHash(userOp, validUntil, validAfter, spentKey, spentMax, allowAnyBundler));
    ...
    function _packContextData(UserOperation calldata userOp, uint32 spentKey, bool allowAnyBundler)
        internal
        pure
    ...
    function _postOp(PostOpMode mode, bytes calldata context, uint256 actualGasCost) internal override {
        (uint256 maxFeePerGas, uint256 maxPriorityFeePerGas, uint32 spentKey, bool allowAnyBundler) =
            abi.decode(context, (uint256, uint256, uint32, bool));
