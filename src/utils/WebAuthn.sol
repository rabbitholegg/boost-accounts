...
import {Base64} from "@solady/contracts/utils/Base64.sol";
import {FCL_Elliptic_ZZ} from "FreshCryptoLib/FCL_elliptic.sol";
import {FCL_ecdsa} from "FreshCryptoLib/FCL_ecdsa.sol";
import {RS256Verify} from "@rs256verify/contracts/RS256Verify.sol";
...
library WebAuthn {
    ...
    function verifyP256Signature(
        ...
        bytes memory challengeBase64 = bytes(Base64.encode(bytes.concat(challenge)));
    ...
    function recover_p256(bytes32 userOpHash, bytes calldata packedSignature) internal view returns (bytes32) {
        ...
        bytes memory challengeBase64 = bytes(Base64.encode(bytes.concat(userOpHash)));
        ...
        bytes32 clientHash = sha256(clientDataJSON);
        bytes32 message = sha256(bytes.concat(authenticatorData, clientHash));
        return TypeConversion.toBytes32(FCL_ecdsa.ec_recover_r1(uint256(message), v, r, s));
    }
